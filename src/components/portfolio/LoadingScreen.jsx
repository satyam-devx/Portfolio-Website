...
export default function LoadingScreen({ onComplete }) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setActiveLetter(l => (l + 1) % LETTERS.length);
    }, 120);
          ...
          <div className="flex gap-2">
            {LETTERS.map((l, i) => (
              <span
                ...
                style={{
                                      color: i === activeLetter ? '#00f5ff' : i < activeLetter || progress > (i / LETTERS.length) * 100 ? 'white' : 'rgba(255,255,255,0.15)',
                                      textShadow: i === activeLetter ? '0 0 20px #00f5ff, 0 0 40px rgba(0,245,255,0.6)' : 'none',
